...
flowchart LR
  ...
  style edge fill:#ede7f6,stroke:#5e35b1
  style app fill:#e8f5e9,stroke:#2e7d32
  style redis fill:#ffebee,stroke:#c62828
  style C fill:#2196f3,stroke:#0d47a1,color:#fff
  style LB fill:#7e57c2,stroke:#311b92,color:#fff
  style I1 fill:#43a047,stroke:#1b5e20,color:#fff
  style I2 fill:#43a047,stroke:#1b5e20,color:#fff
  style In fill:#43a047,stroke:#1b5e20,color:#fff
...
- **Измерение лимита (ключ).** Через **`RATE_LIMIT_KEYS`** задаётся цепочка: **`ip`** (префикс Redis-ключа `ip:…`), **`user_id`** (заголовок, по умолчанию `X-User-ID`, ключ `user:…`), **`api_key`** (заголовок `X-API-Key`, в Redis — `ak:` + SHA-256, без хранения сырого секрета в ключе). Несколько значений через запятую — **последовательные** проверки (`Allow` по каждому измерению); первый отказ даёт 429 без лишних вызовов Redis.
...
- **Ключ лимита:** по **IP**, **user_id** (заголовок) и/или **API key** (SHA-256 в Redis-ключе) — цепочка через `RATE_LIMIT_KEYS`
...
| `RATE_LIMIT_KEYS` | ip | Измерения через запятую: `ip`, `user_id`, `api_key` (порядок = порядок проверок) |
| `RATE_LIMIT_USER_ID_HEADER` | X-User-ID | Заголовок для `user_id` |
| `RATE_LIMIT_API_KEY_HEADER` | X-API-Key | Заголовок для `api_key` |
